...
class NaverCreatorAdvisorKeywordTests(unittest.TestCase):
    ...
    def test_writing_keyword_ui_uses_short_labels_and_search_icon(self) -> None:
        ...
        self.assertIn('text="로워드"', source)
        self.assertIn('fg_color="#03C75A"', source)
        self.assertIn('fg_color="#6C5CE7"', source)
        ...
        self.assertIn("command=self.load_loword_keywords", source)
        self.assertNotIn("command=self.load_google_trends_keywords", source)
